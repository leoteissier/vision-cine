...
const isPatchOpen = ref(props.isOpen);
...
const PatchMovie = async () => {
  try {
    ...
    const mediaId = selectedMedia.value.length > 0 ? await handleMediaUpload(selectedMedia.value) : data.movie.mediaObject?.id || '';
    await patchMovie({validated, ...data.movie, mediaId, selectedActors: selectedActors.value, selectedCategories: selectedCategories.value });
    emits('close');
  } catch (error) {
    console.log(error);
  }
...
const PatchActor = async () => {
  try {
    const validated = false;
    const mediaId = selectedMedia.value.length > 0 ? await handleMediaUpload(selectedMedia.value) : data.actor.mediaObject?.id || '';
    await patchActor({validated, ...data.actor, mediaId, selectedMovies: selectedMovies.value});
    emits('close');
  } catch (error) {
    console.log(error);
  }
...
const PatchCategory = async () => {
  try {
    const validated = false;
    const mediaId = selectedMedia.value.length > 0 ? await handleMediaUpload(selectedMedia.value) : data.category.mediaObject?.id || '';
    await patchCategory({validated, ...data.category, mediaId, selectedMovies: selectedMovies.value});
    emits('close');
  } catch (error) {
    console.log(error);
  }
...
const initFormData = (type: string) => {
  if (!props.itemToPatch) return;

  if (type === 'movie') {
    data.movie = { ...props.itemToPatch, releaseDate: formatDateToInput(props.itemToPatch.releaseDate) };
  } else if (type === 'actor') {
    data.actor = { ...props.itemToPatch, birthdate: formatDateToInput(props.itemToPatch.birthdate) };
  } else if (type === 'category') {
    data.category = { ...props.itemToPatch };
  }
};
...
selectedMovies.value = props.itemToPatch.movies ? props.itemToPatch.movies.collection.map(movie => movie.id) : [];
selectedActors.value = props.itemToPatch.actors ? props.itemToPatch.actors.collection.map(actor => actor.id) : [];
selectedCategories.value = props.itemToPatch.categories ? props.itemToPatch.categories.collection.map(category => category.id) : [];

</script>

<template>
  <UModal v-model="isPatchOpen" prevent-close>
